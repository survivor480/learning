...
/// @return Reference to a newly created object
// ****************************************************************************
CWorksheet & CWorkbook::CreateSheet( const UniString & title, uint32_t frozenWidth, uint32_t frozenHeight, const std::vector<ColumnWidth> & colWidths )
{
...
bool CWorkbook::SaveWorkbook()
{
    char szId[ 16 ] = { 0 };
    {
        ...
        for( std::vector<CWorksheet *>::const_iterator it = m_worksheets.begin(); it != m_worksheets.end(); it++ )
        {
            //sprintf_s( szId, "rId%zu", ( * it )->GetIndex() );
            sprintf_s( szId, "rId%u", unsigned( ( * it )->GetIndex() ) );
            std::stringstream PropValue;
        ...
        if( bFormula )
        {
            //sprintf_s( szId, "rId%zu", id++ );
            sprintf_s( szId, "rId%u", unsigned( id++ ) );
    ...
    }
    {
        ...
        if( ! m_worksheets.empty() || ! m_chartsheets.empty() )
        {
            ...
            {
                //sprintf_s( szId, "rId%zu", i );
                sprintf_s( szId, "rId%u", unsigned( i ) );
                bool Found = false;
                for( std::vector<CWorksheet *>::const_iterator it = m_worksheets.begin(); it != m_worksheets.end(); it++ )
                    if( i == ( * it )->GetIndex() )
                    {
